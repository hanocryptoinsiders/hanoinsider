...
export async function POST(request: Request) {
  try {
    ...
    if (!result.success) {
      const status =
        result.code === "db_error" || result.code === "not_configured" || result.code === "server_config"
          ? 500
    ...
    console.error("[api/crypto/intent]", error);
    return NextResponse.json(
      { error: "Could not start payment. Please try again.", code: "server_error" },
      { status: 500 },
    );
  }
}
